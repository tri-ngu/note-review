...
- **Known limitation — Render**: WebSocket connections to the backend are rejected at the edge (browser: `closed code=1006`; curl: bare `403`) — blocks Game Room. Root-caused: Render's free-tier edge sits behind Cloudflare, and the WS upgrade is blocked there before ever reaching uvicorn — confirmed by comparing response headers: every request that actually reaches the app (health check, even an unmatched path via the SPA catch-all) carries `x-render-origin-server: uvicorn`; the WS 403 never does, with or without an `Origin` header. Not an app-level CORS/Origin check, not fixed by this consolidation, not fixable from `render.yaml`/app code — needs a Render support ticket or a different host. Rest of the pipeline (Upload → generation → flashcard/Review) unaffected. Fly.io and Railway floated as alternatives with cleaner WS pass-through, not yet pursued.
- **Known limitation — prior split-host setup (frontend on Vercel, retired)**: required a manual SPA rewrite (`vercel.json`) since Vercel doesn't infer client-side routing for a Vite SPA on its own, and forced the session cookie into cross-site mode (`SameSite=None; Secure`) since frontend and backend were different origins. Both went away once frontend moved onto Render alongside the backend — but that consolidation only removed the split-host complexity, it didn't touch the WS issue above (confirmed independent of caller origin, reproduced the same way whether the frontend was on Vercel or gone).
