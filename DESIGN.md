...
## Agents (OpenAI Agents SDK, on NVIDIA NIM)
...
`ConceptAllocation`/`ConceptWeight.weight_percentage` values must sum to 100. Rather than always retrying the whole Analyzer call on any drift (a full ~1-3min network round-trip on NIM's free tier), small drift is fixed deterministically in Python: if `abs(sum - 100) <= 10`, every weight is rescaled in place by `100 / sum` (preserves each concept's relative importance ratio) and the call succeeds as-is — no retry, no network cost. Drift beyond ±10 is treated as a genuine Analyzer mistake (e.g. a dropped or duplicated concept, not just rounding noise) and still falls back to `call_agent`'s blind full-call retry. Confirmed via live testing: a 102-vs-100 drift (2% — clearly rounding noise) was the only drift observed in testing so far.

**Provider**: the `openai-agents` framework itself is unchanged, but model calls run against [NVIDIA NIM](https://build.nvidia.com)'s free OpenAI-compatible API (`https://integrate.api.nvidia.com/v1`) instead of OpenAI's own billed models — wired via `set_default_openai_client()` + `set_default_openai_api("chat_completions")` + `set_tracing_disabled(True)`, with each `Agent`'s `model=` wrapped in `OpenAIChatCompletionsModel` (a bare model-name string doesn't work — the SDK parses the `/` in NIM's `vendor/model` slugs as a `litellm/`-style provider prefix and rejects it).

**Model**: `deepseek-ai/deepseek-v4-pro`, picked after live-testing 4 candidates against all 4 real call shapes plus a dedicated Select-All check (see `PROGRESS.md` for the full comparison). Fast (~20-50s/call vs. 90-245s for the runner-up, `nvidia/llama-3.3-nemotron-super-49b-v1.5`) and correctly handles the 1-based `correct_answers` convention. Known flaw, mitigated in Generator/Verifier instructions rather than by switching models: it sometimes bundles multiple correct facts into one option string (e.g. `"Rain and snow"`) instead of giving each fact its own independent option — Generator is now explicitly told never to do this, and Verifier explicitly checks for it.

**Free-tier caveat**: NIM's free tier is rate-limited (~40 RPM) and explicitly not intended for production/customer-facing use. Fine for this project's current stage; revisit if usage grows.
