...
def test_fixture_has_at_least_five_questions():
    assert len(DIGESTIVE_SYSTEM_QUESTION_SET.questions) >= 5


def test_fixture_mixes_multiple_choice_and_select_all():
    flags = {q.is_select_all for q in DIGESTIVE_SYSTEM_QUESTION_SET.questions}
    assert flags == {True, False}


def test_fixture_every_question_has_four_options_and_valid_answers():
    for q in DIGESTIVE_SYSTEM_QUESTION_SET.questions:
